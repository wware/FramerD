...
/* mod_fdserv.c
   Copyright (C) 2002 beingemta, inc.  All Rights Reserved
...
static void *merge_dir_config(apr_pool_t *p,void *base,void *new)
{
  ...
  if (child->n_threads > 0) config->n_threads=child->n_threads;
  else if (parent->n_threads > 0) config->n_threads=parent->n_threads;
  else config->n_threads=-1;
...
static const char *set_n_threads(cmd_parms *parms,void *mconfig,const char *arg)
{
  struct FDSERV_DIR_CONFIG *dconfig=mconfig; int iarg;
  errno=0; iarg=strtol(arg,NULL,10);
  if (errno) return "Invalid number of threads";
  else {
    dconfig->n_threads=iarg;
    return NULL;}
}
...
static const command_rec fdserv_cmds[] =
{
  ...
  AP_INIT_TAKE1("FDServletThreads", set_n_threads , NULL, OR_ALL,
	       "the number of threads the fdservlet should run to handle requests"),
  {NULL}
};
...
{
  int sock;
  const char *sockname=get_sockname(r,r->uri);
...
struct FDSERV_INFO {
  ...
  uid_t uid; gid_t gid; int n_threads;};

static int exec_fdserv(void *child_stuff, child_info *pinfo)
{
  char *argv[5], *envp[5], buf[32];
  ...
  argv[2]=NULL;
  if (info->n_threads > 0) {
    sprintf(buf,"FDSERVLET_THREADS=%d",info->n_threads);
    envp[0]=buf; envp[1]=NULL;}
  else envp[0]=NULL;
  ...
  (request_rec *r,apr_pool_t *p,const char *sockname) 
{
  ...
  info.n_threads=dconfig->n_threads;
...
static void spawn_fdservlet(request_rec *r,apr_pool_t *p,const char *sockname) 
{
  ...
  if (dconfig->n_threads > 0) {
    char *buf=apr_palloc(p,64);
    sprintf(buf,"FDSERVLET_THREADS=%d",dconfig->n_threads);
    envp[0]=buf; envp[1]=NULL;}
  else envp[0]=NULL;
