...
$Id: bignum.c,v 1.6 2007/06/30 16:21:06 haase Exp $
...
/* I made bignum_allocate and bignum_copy into external functions so
   that I could use them from FRAMERD. -Josh
*/

static char vcid[] = "$Id: bignum.c,v 1.6 2007/06/30 16:21:06 haase Exp $";
...
#else
#include "bignum.h"
void fd_raise_exception(char *ex);
#endif
...
extern bignum_type EXFUN (bignum_allocate,
...
static bignum_type EXFUN (bignum_trim,
			  (bignum_type));
extern bignum_type EXFUN (bignum_copy,
			  (bignum_type));
       ...
       bignum_type numerator AND bignum_type denominator)
{
    ...
    return (BIGNUM_MAYBE_COPY (numerator));
  {
      ...
      case bignum_comparison_greater:
	{
      ...
      default:
	fd_raise_exception("bignum comparison invalid");
	return (BIGNUM_OUT_OF_BAND);
      }
  }
}
       ...
       bignum_type numerator AND bignum_type denominator)
{
  ...
  switch (bignum_compare_unsigned (numerator, denominator))
    ...
    case bignum_comparison_greater:
      {
	bignum_type remainder=0; bignum_type quotient=0;
	if ((BIGNUM_LENGTH (denominator)) == 1)
	  {
	    bignum_digit_type digit = (BIGNUM_REF (denominator, 0));
	    if (digit == 1)
       ...
       bignum_type bignum AND long word_length AND int twos_complement_p)
{
  ...
  BIGNUM_ASSERT (n_bits > 0);
  {
    ...
    fast int max_digits = (BIGNUM_BITS_TO_DIGITS (n_bits));
...
unsigned long
DEFUN (bignum_length_in_bytes, (bignum), bignum_type bignum)
{
  if (BIGNUM_ZERO_P (bignum))
    return 0;
  else return BIGNUM_LENGTH(bignum)*sizeof(bignum_digit_type);
}
       ...
       bignum_type u AND bignum_type v AND bignum_type q)
{
  ...
  bignum_digit_type * q_scan= ((BIGNUM_START_PTR (q)) + (BIGNUM_LENGTH (q)));
...
/* The CVS log for this file
   $Log: bignum.c,v $
   Revision 1.6  2007/06/30 16:21:06  haase
   ...
   Fixed big in bignum remainder function
